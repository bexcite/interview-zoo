...
goal is to rotate array A K times; that is, each element of A will be shifted
...
def solution(A, K):
    buf = [None] * K
    # Copy last K elements to temp buffer
    for i in xrange(1, K + 1):
        buf[-i] = A[-i]
    # Shift elements by K, from end of array
    for i in xrange(1, len(A) - K + 1):
        A[-i] = A[-i - K]
    # Copy temp buf to array
    for i in xrange(K):
        A[i] = buf[i]
    return A
