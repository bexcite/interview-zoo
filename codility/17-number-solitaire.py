...
zero-indexed array A of N integers contains the numbers written on the squares.
...
square number 3;
we throw 5; the pebble does not move, since there is no square number 8 on the
board;
we throw 2; the pebble moves to square number 5; we mark this square and the
game ends.
The marked squares are 0, 3 and 5, so the result of the game is 1 + 9 + (-2) =
8. This is the maximal possible result that can be achieved on this board.
...
def solution(A):
    ...
    for i in reversed(range(0, len(A) - 1)):
        costs = []
        for j in xrange(0, 6):
            costs.append(A[i] + get_memoized(i + j + 1))
